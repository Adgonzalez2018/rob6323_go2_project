...
class Rob6323Go2Env(DirectRLEnv):
	...
	def __init__(self, cfg: Rob6323Go2EnvCfg, render_mode: str | None = None, **kwargs):
		...
		self._actions = torch.zeros(self.num_envs, gym.spaces.flatdim(self.singe_action_space), device=self.device)
		...
		self.clock_inputs = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # penalize high torques (compute in _apply_action)
        # store in applied torques in apply action
        ...
        rewards = {
        	...
        	"orient": rew_orient * self.cfg.orient_reward_scale,				# STABILITY
        	"lin_vel_z": rew_lin_vel_z * self.cfg.lin_vel_z_reward_scale,		# STABILITY
        	"ang_vel_xy": rew_ang_vel_xy * self.cfg.ang_vel_xy_reward_scale,	# STABILITY
    ...
    def _reward_raibert_heuristic(self):
    	...
    	# raibert offsets
    	phases = torch.abs(1.0-(self.foot_indices*2.0))*1.0-.5
