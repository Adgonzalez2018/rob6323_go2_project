...
class Rob6323Go2Env(DirectRLEnv):
    ...
    def __init__(self, cfg: Rob6323Go2EnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # part 4 - get specific body indices 
        self._feet_ids = []
        ...
        # vars needed for raibert heuristic
        ...
        obs = torch.cat(
            [
                ...
                for tensor in (
                    ...
                    self._actions,
                    self.clock_inputs    # gait phase info
                )
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # BASE STABILITY SECTION
        # Penalize non-vertical orientation (keep base parallel to ground)
        rew_orient = torch.sum(torch.square(self.robot.data.projected_gravity_b[:, :2]), dim=1)
        # Penalize vertical velocity (reduce bouncing)
        rew_lin_vel_z = torch.square(self.robot.data.root_lin_vel_b[:, 2])
        # Penalize angular vel in XY (reduction in pitch/roll oscillations)
        ...
        # part 4.6 - raibert heuristic
        self._step_contact_targets()  # update gait state
        rew_raibert_heuristic = self._reward_raibert_heuristic()
        
        # part 1.4 - action rate penalization
        ...
        # second derivative (curr - 2*last+2ndlast)
        ...
        # update previous action history (roll buffer and insert new action)
    ...
    def _apply_push_disturbance(self):
        ...
        # find environments where push should be applied
    ...
    def _step_contact_targets(self):
        ...
        smoothing_multiplier_FL = (
            smoothing_cdf_start(torch.remainder(foot_indices[0], 1.0)) * (1 - smoothing_cdf_start(torch.remainder(foot_indices[0], 1.0) - 0.5)) +
            smoothing_cdf_start(torch.remainder(foot_indices[0], 1.0) - 1) * (1 - smoothing_cdf_start(torch.remainder(foot_indices[0], 1.0) - 0.5 - 1))
        )
        smoothing_multiplier_FR = (
            smoothing_cdf_start(torch.remainder(foot_indices[1], 1.0)) * (1 - smoothing_cdf_start(torch.remainder(foot_indices[1], 1.0) - 0.5)) +
            smoothing_cdf_start(torch.remainder(foot_indices[1], 1.0) - 1) * (1 - smoothing_cdf_start(torch.remainder(foot_indices[1], 1.0) - 0.5 - 1))
        )
        smoothing_multiplier_RL = (
            smoothing_cdf_start(torch.remainder(foot_indices[2], 1.0)) * (1 - smoothing_cdf_start(torch.remainder(foot_indices[2], 1.0) - 0.5)) +
            smoothing_cdf_start(torch.remainder(foot_indices[2], 1.0) - 1) * (1 - smoothing_cdf_start(torch.remainder(foot_indices[2], 1.0) - 0.5 - 1))
        )
        smoothing_multiplier_RR = (
            smoothing_cdf_start(torch.remainder(foot_indices[3], 1.0)) * (1 - smoothing_cdf_start(torch.remainder(foot_indices[3], 1.0) - 0.5)) +
            smoothing_cdf_start(torch.remainder(foot_indices[3], 1.0) - 1) * (1 - smoothing_cdf_start(torch.remainder(foot_indices[3], 1.0) - 0.5 - 1))
        )
